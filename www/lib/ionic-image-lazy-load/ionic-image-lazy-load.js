...
.directive('imageLazySrc', ['$document', '$timeout', '$ionicScrollDelegate', '$compile', 'API',
    function ($document, $timeout, $ionicScrollDelegate, $compile, API) {
        return {
            restrict: 'A',
            scope: {
            ...
            link: function ($scope, $element, $attributes) {
                if (!$attributes.imageLazyDistanceFromBottomToLoad) {
                    $attributes.imageLazyDistanceFromBottomToLoad = 100; //50
                }
                if (!$attributes.imageLazyDistanceFromRightToLoad) {
                    $attributes.imageLazyDistanceFromRightToLoad = 100; //50
                }
                ...
                function loadImage() {
                    //Bind "load" event
                    if($scope.logEventCode && !$scope.alreadySent) {
                      API.Insight.log($scope.logEventCode);
